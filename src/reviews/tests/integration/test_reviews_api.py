...
class ReviewsApiTests(TestCase):
    ...
    def test_create_list_update_delete_review_flow(self):
        ...
        review = resp.json()
        review_id = review["id"]
        self.assertEqual(review["accommodation_id"], self.acc.id)
        self.assertEqual(review["author_id"], self.guest.id)
        ...
        self.assertEqual(resp.status_code, 200, resp.content)
        items = resp.json()
        self.assertTrue(any(r["id"] == review_id for r in items), items)
        ...
        self.assertEqual(resp.status_code, 200, resp.content)
        my_items = resp.json()
        self.assertTrue(any(r["id"] == review_id for r in my_items), my_items)

        # 4) Update (only author)
        headers = ensure_csrf(self.client)
        resp = self.client.patch(f"/api/reviews/{review_id}/", {"text": "Updated awesome review!"}, format="json",
                                 **headers)
        self.assertEqual(resp.status_code, 200, resp.content)
        self.assertEqual(resp.json()["text"], "Updated awesome review!")
